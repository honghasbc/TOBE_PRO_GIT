...
$(document).ready(function () {
    ...
    oTable = $('#orderDetailTable').dataTable({
        ...
        "bAutoWidth": false,
        "aaData": ordDetailVOs,
//		"sAjaxSource": '/TOBEPRO/resources/sources/arrays.txt',
...
//	    scope.$apply();
	    window.location.href = "/TOBEPRO/monitor/lineItemDetail";
	});
...
//		oTable.fnDeleteRow(dataRow);
	...
	setEditPanel('cust-info','ajaxUpdateCustInformation');
...
//    	alert("xxxxxxxx"+$scope.item.totNetAmt);
    ...
    $scope.getOrderCmts = function(){
    	if($scope.ord != null && $scope.ord.orderCmtVOs.length == 0) {
    		var loadingCycle = new ajaxLoader("#tabTableId");
...
//apply for phone
function renderEventControl(val){
	//add event keypress for phone
	$("#edit-input-cust-info-phnCntryCd" + val).keyup(function(){
    	autoNextControl(this.value,$("#edit-input-cust-info-phnAreaCd" + val),3);
    });
	$("#edit-input-cust-info-phnAreaCd" + val).keyup(function(){
    	autoNextControl(this.value,$("#edit-input-cust-info-phnNbr" + val),3);
    });
	$("#edit-input-cust-info-phnNbr" + val).keyup(function(){
    	autoNextControl(this.value,$("#edit-input-cust-info-phnExtnNbr" + val),4);
    });
}
